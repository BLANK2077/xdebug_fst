...
// Stable cross-process lease for one logical session name. The lock file is
// outside the removable generation directory, so cleanup cannot invalidate
// exclusion while another process is waiting on the same session id.
