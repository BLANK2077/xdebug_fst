#!/usr/bin/env bash
# Regenerate only the P3-D2 XAMBA APB semantic mirror as a raw FST.
...
readonly FIXTURE_DIR="${REPO_DIR}/testdata/fixtures/apb_xamba_vip"
readonly REQUESTED_WORK_DIR="${XDEBUG_APB_XAMBA_WORK_DIR:-${REPO_DIR}/build/fixtures/apb_xamba_vip}"
...
case "${FIXTURE_DIR}" in
    "${REPO_DIR}"/*) ;;
    *) echo "P3-D2 XAMBA fixture 输出逃逸当前仓库" >&2; exit 2 ;;
esac
case "${WORK_DIR}" in
    "${REPO_DIR}"/*) ;;
    *) echo "P3-D2 XAMBA fixture 构建目录逃逸当前仓库" >&2; exit 2 ;;
esac

[[ -f "${FIXTURE_MANIFEST}" && -f "${DEPENDENCY_LOCK}" ]] || {
    echo "P3-D2 XAMBA fixture 缺少 fixture/tool 身份锁" >&2
    exit 2
}
jq -e --arg revision "$(jq -r '.verilator.revision' "${DEPENDENCY_LOCK}")" \
    ...
    '.build_contract.revision == $revision and
     ...
     .source_contract.original_fixture_id == "xdebug.apb_xamba_vip" and
     ...
     .source_contract.address_formula == "32\u0027h00001000 + index * 4" and
     .source_contract.wait_formula == "index % 4" and
     .source_contract.error_formula == "index % 11 == 0" and
    ...
    echo "P3-D2 XAMBA fixture manifest 与锁定工具/刺激合同不一致" >&2
    exit 2
}
...
(
    ...
    "${VERILATOR_BIN}" -Wno-fatal --cc --exe --build --timing \
        ...
        --top-module xdebug_apb_xamba_fixture_top --Mdir "${OBJECT_DIR}" \
        -CFLAGS "${XDEBUG_FIXTURE_PREFIX_MAP_FLAGS[*]}" \
        "testdata/fixtures/apb_xamba_vip/xdebug_apb_xamba_fixture_top.sv" \
        "testdata/fixtures/apb_xamba_vip/tb_apb_xamba.cpp"
)
"${OBJECT_DIR}/Vxdebug_apb_xamba_fixture_top" "${GENERATED_FST}"

[[ -s "${GENERATED_FST}" ]] || {
    echo "P3-D2 XAMBA fixture 未生成非空 FST" >&2
    exit 1
}
...
[[ "${ACTUAL_FST_SHA256}" == "${EXPECTED_FST_SHA256}" &&
   "${ACTUAL_FST_SIZE}" == "${EXPECTED_FST_SIZE}" ]] || {
    echo "P3-D2 XAMBA FST 与确定性输出锁不一致: ${ACTUAL_FST_SHA256}/${ACTUAL_FST_SIZE}" >&2
    exit 1
}
...
(
    cd -- "${FIXTURE_DIR}"
    sha256sum xdebug_apb_xamba_fixture_top.sv tb_apb_xamba.cpp \
