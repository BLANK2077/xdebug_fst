...
| Rust 依赖 | rust/Cargo.lock、licenses/rust-dependencies.json | 锁文件全部组件审查；打包时另外解析实际 runtime 依赖闭包并携带源码、许可及 SPDX |
| GCC libstdc++ / libgcc | GCC 13.3.1、GPL-3.0-with-GCC-exception | 核对实际库来源；随附 GPL、Runtime Library Exception 和精确对应源码，缺对应材料不得标为公开可发布 |
...
- **GCC 对应源码**：必须核对复制到工具包的实际运行库与公开源码版本、发行补丁的一致性。
