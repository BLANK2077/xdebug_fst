...
- [x] 按 1K→64K 依次执行完整矩阵。
- [x] 若某规模失败，保存阶段和诊断，不跳过、不 fallback。
- [x] 修复 session 名称、展开路径锚点和 UDS 短路径问题，并重跑受影响规模。
- [x] 未因脚本变化重建现有 fixture cache。

计划提交：仅在实现需要修复时提交；原始生成物不提交。

### 阶段 4：结果分析与验收

- [x] 汇总各阶段 wall time、max RSS、产物大小和相邻规模倍率。
- [x] 区分源码规模、展开实例规模、time-point 数和 FST 变化量对性能的影响。
- [x] 标记近线性、超线性、固定成本和异常点。
- [x] 给出当前可接受边界、主要瓶颈和下一步优化建议。
- [x] 运行生成器单测、全量 pytest、静态路径门禁和仓库状态检查。
...
- 2026-08-19：阶段 3–4 实测完成。七档全部成功；64K Verilator 前端 1769.09 s、
  DesignDB G++ 170.38 s/峰值 6,553,748 KiB、`.so` 346,661,424 bytes；session 与六项
  Action 全部成功。正式首次运行还暴露长输出目录导致 UDS 超限，runner 改用短临时 HOME 后
  从全新目录完整重跑，没有改变被测数据面或重建 fixture cache。详细结论见性能报告。
- 2026-08-19：最终验收完成。445 项 pytest 全部通过；七档 42 项 Action、21 个产物 SHA、
  精确行数和完整性合同审计通过；新增/报告与本地结果路径门禁通过；`testdata` 无差异，
  fixture cache 未修改。
