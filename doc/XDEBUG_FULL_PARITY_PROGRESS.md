...
- xdebug-fst 当前功能与验收提交：`b6f2617`；精确表达式 `case matches` 的 Verilator 修改前证据为 `ea1d3c9b4`、最小实现为 `adc193c2f`；同值 NBA 事件依赖的 Verilator 修改前证据为 `01f9f2a4b`、最小实现为 `6239de45e`；复杂 output 表达式修改前失败证据为 `ae76785`，常量 NBA 修复为 `f3b5143`，条件 output 修改前失败证据为 `b46b5cd`；过程/常量、多级端口、固件、架构说明与全量门禁证据均已登记
