...
- P5 stream 基础批次：`tests/test_stream.py` 12/12 与 `test_value_at_stream_source` 1/1 通过，真实从 `stream/waves.fst` 读取四笔 40/60/80/120ps、AA/BB/CC/DD transfer，覆盖冻结命名配置、describe、summary、transfer_window、line_limit 裁剪、动态 validate 和显式最终 export；随后 8/8 CTest 与冻结基线检查通过。实现只在请求期间由 Wellen 直接读取当前 FST，`cache_scope` 仅作为扫描范围选择，不建立离线或跨请求波形缓存，Verilator 仓库未修改。packet、stall、filter 与复杂 beat field 表达式仍待后续独立批次，尚未宣称 stream 全合同关闭。
