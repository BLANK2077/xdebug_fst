...
## 0.1.0 正式发布任务（2026-09-07）

维护者已明确授权正式发布、推送远端并在推送完成后公开仓库；正式版本选择 0.1.0，保留 0.1.0-rc.1 标签。按个人开发者流程执行，不走 PR。维护者确认自建 XAMBA VIP；该确认不改写 SVT 历史对照数据的来源，也不代替第三方许可证材料。

### 实施顺序与验收标准

1. 将计划和进度写入本任务书，随后建立独立 goal。保留已有 fixture、工具链、依赖与构建缓存。
2. 默认 README 改为英文，顶部链接 README.zh-CN.md；两份文档互链，示例版本统一为 0.1.0。二进制包同时包含两份 README，保留 patched Verilator 二进制及配套文件。更新个人授权与第三方来源审查记录。
3. 分阶段详细中文提交，从干净提交构建内嵌正式版本和真实 revision 的二进制。使用既有锁定 Rust 1.97.1/GCC 13.3.1 环境；不通过重命名 RC 冒充正式版本。
4. 生成 Linux x86_64 二进制、项目源码、第三方对应源码、SPDX、环境记录、验证证据及 SHA256。普通/ASan/UBSan 门禁、73 Action 审计、EL8/Ubuntu 22.04/24.04 实际安装及长波形检查全部通过。报告必须对应实际版本，不复用旧报告充当新验收；保留已说明的 Python 辅助进程 sanitizer 例外。
5. 推送工作分支并快进现默认分支，不改分支名、不强推、不重写历史。若远端出现并发变更，合并后重新验证受影响内容。创建 0.1.0 标签和草稿，上传附件后下载核验，发布为正式 Latest。
6. 确认默认分支英文 README 已更新后将仓库公开；匿名检查首页、中文 README、Release 和下载 SHA256。记录最终提交、标签、链接、公开状态，提交并推送收尾账本，完成 goal。
